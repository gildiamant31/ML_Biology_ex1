...
cancer_patient <-select(cancer_patient, -c(`Dust Allergy`,,`OccuPational Hazards`,`Genetic Risk`,`Chest Pain`))
...
```
pca gave as moderate results, lets check our vatiance in the 4 firsts pcs
```{r}
# calculate variance of the 4 firsts pcs
print(round(prcomp_cancer_patient$sdev[1:4] / sum(prcomp_cancer_patient$sdev) * 100, 2))
```
we didnt got high precentage in the variace, so its maybe the cause of the  moderate results
...
```{r}
c_p_svm = as.matrix(cancer_patient_z[1:19])
# fit model
svmfit <- svm(x=c_p_svm, y=cancer_patient_z$Level, kernel = "radial", cost = 10, gamma = 1)
# plot results
plot(svmfit, cancer_patient_z)
```
...
```{r}
# fit model
classifier <- svm(Level~ ., data = cancer_n_train[1:24], kernel = "radial", cost = 10, gamma = 1)
print(svmfit)
svm_pred = predict(classifier, newdata = cancer_n_test[1:24])
...
#knn_model contains only preicted value
test_v_1 <- cbind(cancer_n_test,svm_pred)
#This will return TRUE or FALSE.
...
```
 ...
 ניסיתי את הדבר הבא וזה לא הצליח!!!
...
```{r}
# fit and plot
kernfit <- ksvm(as.matrix(cancer_n_train[1:23]),cancer_n_train$Level, type = "C-svc", kernel = 'rbfdot', 
                C = 100, scaled = c())

# Create a fine grid of the feature space
x.1 <- seq(from = min(cancer_n_train$PC1), to = max(cancer_n_train$PC1), length = 100)
x.2 <- seq(from = min(cancer_n_train$PC2), to = max(cancer_n_train$PC2), length = 100)
x.grid <- expand.grid(x.2, x.1)

# Get class predictions over grid
kern_pred <- predict(kernfit, newdata = cancer_n_test[1:24])
...
cancer_model <- C5.0(cancer_n_train[1:7], cancer_train_labels)
...
cancer_pred <- predict(cancer_model, cancer_n_test[1:23])
...
test_merged1 <- cbind(cancer_n_test,cancer_pred)
...
#setting a seed for reproducible results
set.seed(1234)
cancer_boost10 <- C5.0(cancer_n_train[1:23], cancer_train_labels, trials = 10)
...
cancer_boost_pred10 <- predict(cancer_boost10, cancer_n_test) 
# present it on Crossable
CrossTable(cancer_test_labels, cancer_boost_pred10,
prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
dnn = c('actual default', 'predicted default'))

#DT_Model merge
#pred10 contains only predicted value
test_merged2 <- data.frame(cancer_n_test,cancer_boost_pred10,cancer_test_labels)
test_merged2$Correct <- test_merged2$cancer_test_labels == 
  test_merged2$cancer_boost_pred10 #This will return TRUE or FALSE.

# again here we also plot the types according two specific columns after PCA
c2 <-ggplot(test_merged2, aes(y=PC1, x = PC2, col=cancer_test_labels, label = Correct)) +
  geom_point(size=5, alpha = 0.2) + 
  theme_moma() +
  theme(legend.position = "bottom") +
