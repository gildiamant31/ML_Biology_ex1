...
Lets check the almost even distribution between labeles was preserved:
```{r}
...
We saw very great results in knn and radial svm, but moderate separation in the pca(althogh pca its not calsifier, we expect to see good seperation of the samples by class which is not so good). We suggest that it may cause by the data which is cannot be separted lineary, o we try linear svm.
...
#some explanation on lda and what we want to achive
