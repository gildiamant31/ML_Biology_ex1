...
```{r}
 ...
 # create two plots
 pM = ggplot(cancer_patient_gender[cancer_patient_gender$Gender=="Male",], aes(x = factor(1) , fill = Level)) +
 geom_bar(width = 1) + facet_grid(cols = vars(Gender)) + coord_polar(theta = "y")+ xlab("") + ylab("") 
 pF = ggplot(cancer_patient_gender[cancer_patient_gender$Gender=="Female",], aes(x = factor(1) , fill = Level)) +
 geom_bar(width = 1) + facet_grid(cols = vars(Gender)) + coord_polar(theta = "y")+ xlab("") + ylab("")  + ggtitle("")
 # show plots together
 title <- ggdraw() + 
...
```
All of the patients were predicted correctly. 

Lets check the results for range of K hyper-parameter for KNN -
```{r}
set.seed(1234)
...
```
As we saw, we can use KNN to as multi-class classifier algorithm. If we use 'k' in values of 1 - 9 then the KNN got excellent results with zero mistakes.
