...
cancer_patient_sel <-select(cancer_patient, -c(`OccuPational Hazards`,`Alcohol use`))
correlationMatrix <- cor(cancer_patient_sel[,2:22])
highlyCorrelated <- findCorrelation(correlationMatrix, cutoff=0.85)
# print indexes of highly correlated attributes
print(highlyCorrelated)
```
...
# plot data set
ggplot(data = pca_cancer_patient, aes(x = PC1, y = PC2, color = Level, shape = Level)) + 
  geom_point(size = 2) +
