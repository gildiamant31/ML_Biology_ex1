...
- the movies in languages of countries with more western culture (English,French,Spanish,German) tend to be with length of 1-2 hours while movies with other languages are have also significant piece of longer(>2) movies.
that may point out that the western culture are tend to be less patient to see a long movie
```{r}
surveys %>% group_by(Languages) %>%  tally(sort = TRUE) %>% head(10)

surveys_fixed <- surveys

surveys_fixed$Runtime[surveys$Runtime == "1-2 hour"] <- "1.5"
surveys_fixed$Runtime[surveys$Runtime == "< 30 minutes"] <- "0.3"
surveys_fixed$Runtime[surveys$Runtime == "> 2 hrs"] <- "2.5"
surveys_fixed$Runtime[surveys$Runtime == "30-60 mins"] <- "0.7"
surveys_fixed$Runtime <- as.numeric(surveys_fixed$Runtime)
...
ggplot(data = one_lan, aes(x=Languages, y=Runtime, fill=Languages)) + geom_boxplot()+ geom_point()+
  theme_bw() +
...
We also cheked the number of movies and Series and got that while there is more movies then series, the difference in the revenues is not cause mainly because of that
```{r}
# convert the income to integer
...
# TODO NA
#number_of<-Boxoffice_fixed %>% group_by(`Series or Movie`) %>%  tally(sort = TRUE)
...
surveys_fixed_no_NA_Genres$Genre <- sub('.*,\\s*', '', surveys_fixed_no_NA_Genres$Genre)
ggplot(data = surveys_fixed_one_genre_no_NA_Genre) +
  geom_col(mapping = aes(y =Genre,x = `IMDb Score`)) + labs(title= "Average IMDB score per Genre")
```
...
surveys_fixed_one_genre_no_NA_Genre <- surveys_fixed_one_genre_no_NA_Genre %>% separate(`Release Date`, c("day", "month", "year"), sep = " ", convert = TRUE)
...
To check the relation between three different sites' scores
```{r}
...
ggplot(data = surveys) +
  geom_histogram(mapping = aes(x = `IMDb Score`), binwidth = 0.5)
