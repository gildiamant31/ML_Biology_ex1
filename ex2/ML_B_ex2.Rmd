...
# again here we also plot the types according two specific columns after PCA
c1 <-ggplot(test_v_1, aes(y=PC1, x = PC2, col=Type, label = Correct)) +
  geom_point(size=5, alpha = 0.2) + 
  theme_moma() +
  theme(legend.position = "bottom") +
  ggtitle("KNN Prediction Results") + 
  geom_text(size = 3, show.legend = FALSE)
c2 <-ggplot(test_merged2, aes(y=PC1, x = PC2, col=Type, label = Correct)) +
  geom_point(size=5, alpha = 0.2) + 
  theme_moma() +
  theme(legend.position = "bottom") +
  ggtitle("Improved DT Prediction Results") + 
...
After we analyzed and run different algorithms on our data we assume that we got very good and similar classification results for each algorithm with and without any improvement maybe as a result of that small size of our dataset cause small group of test sampels which maybe is easy to classify. Another option is that our dataset is easy to cllasify dou to  differentiated  features.
