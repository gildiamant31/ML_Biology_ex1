...
surveys= surveys[sample(1:nrow(surveys)), ]
```
...
```{r}
# sample 199 observations out of the total 400
train_sample <- sample(400, 170)

str(train_sample) # the resulting train_sample object is a vector of 800 random integers

# split into train/test
surveys_train <- surveys[train_sample, ]
surveys_test <- surveys[-train_sample, ]

# check that we got about 30% defaulted loans in each dataset:
prop.table(table(surveys_train$Type))
prop.table(table(surveys_test$Type))
```
...
surveys_model <- C5.0(surveys_train[-8], surveys_train$Type)
...
surveys_pred <- predict(surveys_model, surveys_test)

CrossTable(surveys_test$Type, surveys_pred, prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE, dnn = c('actual Type', 'predicted Type'))
```
...
surveys_boost10 <- C5.0(surveys_train[-8], surveys_train$Type, trials = 10)

surveys_boost10

summary(surveys_boost10)
```
...
surveys_boost_pred10 <- predict(surveys_boost10, surveys_test)

CrossTable(surveys_test$Type, surveys_boost_pred10,
