...
2. Your conclusion should include a summary of what you did, which model is better and which hyperparameters you recommend
...
## knn:
# TODO לחפור עוד על KNN
knn is a non-linear clasiffier.
...
Lets try normalization instead of  z-score standardization :
...
we already have training and test sets, normolized and shuffeled, lets check the almost even distribition between labales was preserve:
```{r}
...
training data as expected,and also a bit worse than the knn classifier.
...
set.seed(234)
seeds_folds <- vfold_cv(seeds_train) # create the actual cross-validation folds
```
