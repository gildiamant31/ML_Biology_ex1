...
pca_seeds$Type<-as.factor(pca_seeds$Type)
```
...
seeds_z <- as.data.frame(scale(pca_seeds[1:2]))
...
# labels of train set
seeds_train_labels <- pca_seeds$Type[1:160]
# labels of test set
seeds_test_labels <- pca_seeds$Type[161:199]
...
test_v_1 <- data.frame(seeds_test_z,seeds_test_pred_z,seeds_test_labels)
#This will return TRUE or FALSE.
test_v_1$Correct <- test_v_1$seeds_test_labels == test_v_1$seeds_test_pred_z
...
There were four mistakes, two of type 2 and one of type 3 were mistakenly predicted as type 1.
...
seeds_n <- as.data.frame(lapply(pca_seeds[1:2], normalize))
...
```
Here we can see a worse predictions than before, another mistake has been added and now we have 5 prediction mistakes.  
...
Use KNN with k=5 got only one mistake as we can see here :

```{r}
knn_pred_k5 <- knn(train=seeds_n_train, test=seeds_n_test, cl=seeds_train_labels, k=5)
a<-CrossTable(x = seeds_test_labels, y = knn_pred_k5, prop.chisq=FALSE)
```
...
You can see on the last plot, that the best K hyper-parameter to use for this  
dataset is 5 with the biggest accuracy.
...
test_merged1 <- data.frame(seeds_n_test,seeds_pred,seeds_test_labels)
test_merged1$Correct <- test_merged1$seeds_test_labels == 
  test_merged1$seeds_pred #This will return TRUE or FALSE.

# again here we also plot the types according two specific columns after PCA
c1 <-ggplot(test_merged1, aes(y=PC1, x = PC2, col=seeds_test_labels, label = Correct)) +
  geom_point(size=5, alpha = 0.2) + 
  theme_moma() +
  theme(legend.position = "bottom") +
...
# remove from here the summary - irrelevant and long
...
c2 <-ggplot(test_merged2, aes(y=PC1, x = PC2, col=seeds_test_labels, label = Correct)) +
  ...
  geom_text(size = 3, show.legend = FALSE)
```
we got small improvement in the results with only 4/39 mistaken classification 
...
#knn_model contains only preicted value
test_v_1 <- data.frame(seeds_n_test,knn_pred_k5,seeds_test_labels)
#This will return TRUE or FALSE.
test_v_1$Correct <- test_v_1$seeds_test_labels == test_v_1$knn_pred_k5 

# again here we also plot the types according two specific columns after PCA
c1 <-ggplot(test_v_1, aes(y=PC1, x = PC2, col=seeds_test_labels, label = Correct)) +
  geom_point(size=5, alpha = 0.2) + 
  theme_moma() +
  theme(legend.position = "bottom") +
  ggtitle("KNN Prediction Results") + 
...
After we analyzed and run different algorithms on our data we assume that we got very good and similar classification results for each algorithm with and without any improvement as a result of the small size of our dataset.

first we use PCA to reduce our data dimension.
After that we run knn and got better predictions with z-score normalization (we chose K=2).
The best prediction result that we got was over 96 accuracy in predictions, while using hyper-parameter K=5(while using min-max normalization).

The next algorithm was DT we use it on our dataset after normalization process(min-max) without improvement and got 5 mistakes.
After boost improvement (10 trails of the algorithm) on DT algorithm we got only 4 mistakes in algo's predictions.
We used the default Decision Tree and its default hyper-parameters.

After comparing two algorithms results (see the two comparing plots) we assume that KNN with K=5 as hyper-parameter is the the better algorithm to choose while using this data.